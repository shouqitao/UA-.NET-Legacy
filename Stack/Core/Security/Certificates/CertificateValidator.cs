...
namespace Opc.Ua {
    ...
    public class CertificateValidator {
            ...
            try {
                ...
                lock (m_callbackLock) {
                    if (m_CertificateValidation != null) {
                        CertificateValidationEventArgs args =
                            new CertificateValidationEventArgs(new ServiceResult(e), certificate);
                ...
                if (!accept) {
                    ...
                    lock (m_lock) {
                        Utils.Trace((int) Utils.TraceMasks.Error, "Certificate '{0}' rejected. Reason={1}",
                            certificate.Subject, (StatusCode) e.StatusCode);

                        if (m_rejectedCertificateStore != null) {
                            Utils.Trace((int) Utils.TraceMasks.Error, "Writing rejected certificate to directory: {0}",
                                m_rejectedCertificateStore);
        ...
        private X509SubjectKeyIdentifierExtension FindSubjectKeyIdentifierExtension(X509Certificate2 certificate) {
            for (int ii = 0; ii < certificate.Extensions.Count; ii++) {
                X509SubjectKeyIdentifierExtension extension =
                    certificate.Extensions[ii] as X509SubjectKeyIdentifierExtension;
                ...
                for (int ii = 0; ii < explicitList.Count; ii++) {
                    ...
                    if (issuer != null) {
                        ...
                        if (Match(issuer, subjectName, serialNumber, keyId)) {
                            ...
                            return new CertificateIdentifier(issuer,
            ...
            lock (m_lock) {
                ...
                if (issuedByCA && !isIssuerTrusted) {
                    if (m_applicationCertificate == null ||
                        !Utils.IsEqual(m_applicationCertificate.RawData, certificate.RawData)) {
                        throw ServiceResultException.Create(
                            StatusCodes.BadCertificateUntrusted,
                            "Certificate issuer is not trusted.\r\nSubjectName: {0}\r\nIssuerName: {1}",
        ...
        private static ServiceResult CheckChainStatus(X509ChainStatus status, CertificateIdentifier id,
            CertificateIdentifier issuer, bool isIssuer) {
            switch (status.Status) {
                ...
                case X509ChainStatusFlags.RevocationStatusUnknown: {
                    ...
                    if (id.Certificate != null &&
                        Utils.CompareDistinguishedName(id.Certificate.Subject, id.Certificate.Subject)) {
                        break;
                    }
            ...
            public override void Validate(X509Certificate2 certificate) {
                try {
                    ...
                    throw new System.IdentityModel.Tokens.SecurityTokenValidationException(
                        "Could not validate certificate.", e);
                }
            }
    ...
    /// </summary>
    public delegate void CertificateValidationEventHandler(CertificateValidator sender,
