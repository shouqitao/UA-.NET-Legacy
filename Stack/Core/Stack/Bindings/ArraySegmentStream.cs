    ...
    public class ArraySegmentStream : Stream {
        ...
        public override int Read(byte[] buffer, int offset, int count) {
            ...
            while (count > 0) {
                ...
                if (bytesLeft > count) {
                    Array.Copy(m_currentBuffer.Array, m_currentPosition + m_currentBuffer.Offset, buffer, offset,
                        count);
                    bytesRead += count;
        ...
        public override void Write(byte[] buffer, int offset, int count) {
            while (count > 0) {
                ...
                // copy the bytes requested.
                if (bytesLeft >= count) {
                    Array.Copy(buffer, offset, m_currentBuffer.Array, m_currentPosition + m_currentBuffer.Offset,
                        count);
