...
namespace Opc.Ua.Bindings {
    ...
    public class TcpMessageSocket : IDisposable {
        ...
        public IAsyncResult BeginConnect(Uri endpointUrl, AsyncCallback callback, object state) {
            ...
            lock (m_socketLock) {
                ...
                if (!ipV6Required) {
                    ...
                    if (ipV6Required) {
                        for (int ii = 0; ii < localAddresses.Length; ii++) {
                            if (localAddresses[ii].AddressFamily == AddressFamily.InterNetwork &&
                                !IPAddress.IsLoopback(localAddresses[ii])) {
                                ipV6Required = false;
                                break;
                            }
                        }
        ...
        /// </summary>
        public IAsyncResult BeginWriteMessage(ArraySegment<byte> buffer, int timeout, AsyncCallback callback,
            object state) {
        ...
        private void WriteMessage(WriteOperation operation) {
            ...
            if (socket == null) {
                operation.Fault(ServiceResult.Create(StatusCodes.BadConnectionClosed,
                    "Socket is no longer available."));
                return;
            }
            ...
            try {
                ...
                if (bytesSent < buffers.TotalSize) {
                    operation.Fault(ServiceResult.Create(StatusCodes.BadConnectionClosed,
                        "Write operation could not complete."));
                    return;
                }
