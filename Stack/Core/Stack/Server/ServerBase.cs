...
namespace Opc.Ua {
    ...
    public partial class ServerBase : IServerBase, IDisposable {
        ...
        public ServiceMessageContext MessageContext {
            get { return (ServiceMessageContext) m_messageContext; }
        ...
        public ServiceResult ServerError {
            get { return (ServiceResult) m_serverError; }
        ...
        protected void InitializeRequestQueue(ApplicationConfiguration configuration) {
            ...
            m_requestQueue =
                new RequestQueue(this, minRequestThreadCount, maxRequestThreadCount, maxQueuedRequestCount);
        }
        ...
        public CertificateValidator CertificateValidator {
            get { return (CertificateValidator) m_certificateValidator; }
        ...
        protected X509Certificate2 InstanceCertificate {
            get { return (X509Certificate2) m_instanceCertificate; }
        ...
        protected X509Certificate2Collection InstanceCertificateChain {
            get { return m_instanceCertificateChain; }
        ...
        protected ServerProperties ServerProperties {
            get { return (ServerProperties) m_serverProperties; }
        ...
        protected ApplicationConfiguration Configuration {
            get { return (ApplicationConfiguration) m_configuration; }
        ...
        protected ApplicationDescription ServerDescription {
            get { return (ApplicationDescription) m_serverDescription; }
            ...
            for (int ii = 0; ii < baseAddresses.Count; ii++) {
                ...
                if (requireEncryption) {
                    ...
                    if (InstanceCertificateChain != null) {
                        ...
                        System.Xml.XmlElement configExtension =
                            configuration.Extensions.Find(e => e.Name.Equals("SendCertificateChain"));
                        if ((configExtension == null ||
                             (configExtension != null && configExtension.InnerText.Equals("False")))) {
            ...
            for (int ii = 0; ii < baseAddresses.Count; ii++) {
                ...
                if (uri.Scheme == Utils.UriSchemeHttps) {
                    ...
                    if (InstanceCertificateChain != null) {
                        ...
                        if (configExtension != null && "true".Equals(configExtension.InnerText,
        ...
        protected virtual UserTokenPolicyCollection GetUserTokenPolicies(ApplicationConfiguration configuration,
        ...
        private string GetBestDiscoveryUrl(Uri clientUrl, BaseAddress baseAddress) {
            ...
            if (baseAddress.ProfileUri == Profiles.WsHttpXmlOrBinaryTransport ||
                baseAddress.ProfileUri == Profiles.WsHttpXmlTransport) {
                url += "/discovery";
            }
            ...
            foreach (EndpointDescription endpoint in endpoints) {
                ...
                foreach (BaseAddress baseAddress in baseAddresses) {
                    ...
                    if ((endpoint.TransportProfileUri == Profiles.HttpsBinaryTransport &&
                         baseAddress.ProfileUri == Profiles.HttpsXmlOrBinaryTransport)
                    ...
                    if ((endpoint.TransportProfileUri == Profiles.HttpsXmlTransport &&
                         baseAddress.ProfileUri == Profiles.HttpsXmlOrBinaryTransport)
                        || endpoint.TransportProfileUri == Profiles.HttpsXmlTransport &&
                        baseAddress.ProfileUri == Profiles.HttpsXmlTransport) {
                        translateHttpsEndpoint = true;
                    }
                    ...
                    if (endpointUrl.Path.StartsWith(baseAddress.Url.PathAndQuery) &&
        ...
        protected virtual ResponseHeader CreateResponse(RequestHeader requestHeader, Exception exception) {
            ...
            responseHeader.ServiceDiagnostics = new DiagnosticInfo(exception,
                (DiagnosticsMasks) requestHeader.ReturnDiagnostics, true, stringTable);
                ...
                lock (m_lock) // i.e. Monitor.Enter(m_lock)
                {
                    ...
                    if (m_stopped || m_queue.Count >= m_maxRequestCount) {
                        ...
                        // start a new thread to handle the request if none are idle and the pool is not full.
                            ...
                            Utils.Trace("Thread created: " + Thread.CurrentThread.ManagedThreadId +
                                        ". Current thread count: " + m_totalThreadCount + ". Active thread count" +
                                        m_activeThreadCount);
                        }
                    }
                }
                    ...
                    while (true) {
                        ...
                        while (m_queue.Count == 0) {
                            ...
                            if (m_stopped || (!Monitor.Wait(m_lock, 30000) && m_totalThreadCount > m_minThreadCount)) {
                                ...
                                Utils.Trace("Thread ended: " + Thread.CurrentThread.ManagedThreadId +
                                            ". Current thread count: " + m_totalThreadCount + ". Active thread count" +
                                            m_activeThreadCount);
