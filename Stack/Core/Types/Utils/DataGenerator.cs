...
namespace Opc.Ua.Test {
    ...
    public class DataGenerator {
        ...
        /// </summary>
        private Variant[] GetRandomArrayInVariant(BuiltInType builtInType, bool useBoundaryValues, int length,
            bool fixedLength) {
        ...
        public ExpandedNodeId GetRandomExpandedNodeId() {
            ...
            ushort serverIndex = m_serverUris.Count == 0
                ? (ushort) 0
                : (ushort) m_random.NextInt32(m_serverUris.Count - 1);
        ...
        public StatusCode GetRandomStatusCode() {
            int offset = GetRandomRange((int) (StatusCodes.BadUnexpectedError >> 16),
                (int) (StatusCodes.BadMaxConnectionsReached >> 16));
        ...
        private static readonly BoundaryValues[] s_AvailableBoundaryValues = new BoundaryValues[] {
            ...
            new BoundaryValues(typeof(float), Single.Epsilon, Single.MaxValue, Single.MinValue, Single.NaN,
                Single.NegativeInfinity, Single.PositiveInfinity, (float) 0),
            new BoundaryValues(typeof(double), Double.Epsilon, Double.MaxValue, Double.MinValue, Double.NaN,
                Double.NegativeInfinity, Double.PositiveInfinity, (double) 0),
            new BoundaryValues(typeof(string), null, String.Empty),
            new BoundaryValues(typeof(DateTime), DateTime.MinValue, DateTime.MaxValue, new DateTime(1099, 1, 1),
                new DateTime(2039, 4, 4), new DateTime(2001, 9, 11, 9, 15, 0, DateTimeKind.Local)),
            new BoundaryValues(typeof(Guid), Guid.Empty), new BoundaryValues(typeof(Uuid), Uuid.Empty),
            new BoundaryValues(typeof(byte[]), null, new byte[0]), new BoundaryValues(typeof(XmlElement), null),
            new BoundaryValues(typeof(NodeId), null, NodeId.Null, new NodeId(Guid.Empty), new NodeId(String.Empty),
                new NodeId(new byte[0])),
            new BoundaryValues(typeof(ExpandedNodeId), null, ExpandedNodeId.Null, new ExpandedNodeId(Guid.Empty),
