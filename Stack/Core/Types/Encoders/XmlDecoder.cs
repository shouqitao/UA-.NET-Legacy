    ...
    public class XmlDecoder : IDecoder, IDisposable {
        ...
        public object ReadVariantContents(out TypeInfo typeInfo) {
            ...
            try {
                ...
                throw new ServiceResultException(
                    ...
                    Utils.Format("Element '{1}:{0}' is not allowed in an Variant.", m_reader.LocalName,
        ...
        public XmlElement ReadXmlElement(string fieldName) {
            if (BeginField(fieldName, true)) {
                if (MoveToElement(null)) {
                    ...
                    XmlDocument document = new XmlDocument();
                    XmlElement value =
                        document.CreateElement(m_reader.Prefix, m_reader.LocalName, m_reader.NamespaceURI);
        ...
        public IEncodeable ReadEncodeable(
            ...
            if (BeginField(fieldName, true)) {
                ...
                while (!(m_reader.NodeType == XmlNodeType.EndElement && m_reader.LocalName == fieldName &&
                         m_reader.NamespaceURI == m_namespaces.Peek())) {
                    if (m_reader.NodeType == XmlNodeType.None) {
                        throw new ServiceResultException(
                            StatusCodes.BadDecodingError,
                            Utils.Format("Unexpected end of stream decoding field '{0}' for type '{1}'.", fieldName,
                                systemType.FullName));
                    }
        ...
        private bool BeginField(string fieldName, bool isOptional, out bool isNil) {
            ...
            if (!m_reader.IsStartElement(fieldName, m_namespaces.Peek())) {
                if (!isOptional) {
                    throw new ServiceResultException(
                        StatusCodes.BadDecodingError,
                        Utils.Format("Encountered element: '{1}:{0}' when expecting element: '{2}:{3}'.",
                            m_reader.LocalName, m_reader.NamespaceURI, fieldName, m_namespaces.Peek()));
                }
        ...
        private void EndField(string fieldName) {
            if (!String.IsNullOrEmpty(fieldName)) {
                ...
                if (m_reader.NodeType != XmlNodeType.EndElement || m_reader.LocalName != fieldName ||
                    m_reader.NamespaceURI != m_namespaces.Peek()) {
                    throw new ServiceResultException(
                        StatusCodes.BadDecodingError,
                        Utils.Format("Encountered end element: '{1}:{0}' when expecting element: '{3}:{2}'.",
                            m_reader.LocalName, m_reader.NamespaceURI, fieldName, m_namespaces.Peek()));
                }
